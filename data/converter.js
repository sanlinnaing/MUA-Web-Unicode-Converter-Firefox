 ...
 */
 function convertTree(parent) {
 	...
 	var pstyle=""+parent.className;  // firefox parent.className is not a function error fix
 	if (parent.className != null && pstyle.search('_c_o_nvert_') != -1) {
		...
		if (child.nodeType != Node.TEXT_NODE && child.hasChildNodes()) {
			...
			if (text && isMyanmar(text)) {
				...
				if (shouldIgnoreNode(parent) == false && isZawgyi(text)) {
					...
					var cstyle="" + parent.className;
					if (parent.className == null || cstyle.search('_c_o_nvert_') == -1) {
...
if(isMyanmar(document.title)){
	if(isZawgyi(document.title)){
		document.title=Z1_Uni(document.title);
	}
}
var list = document.querySelector('body');
if (!list) {
